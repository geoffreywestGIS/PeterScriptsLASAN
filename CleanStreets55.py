...
try:
    ...
    #Selection Query
    Harborselection = "PtrText IS NULL AND RESOLUTION_CODE <> '0' AND RESOLUTION_CODE IS NOT NULL  AND Prior_RESOLUTION_CODE = '55' AND last_edited_date >=  DATEADD(hour, -12,  GETDATE())"
    ...
    arcpy.AddField_management(PeterTable, "CleanStreetsPeter55", "TEXT")

    dateExpression = "Date"
    arcpy.CalculateField_management(PeterTable, "DATE", dateExpression)

    #Calculates Field with expression for Peter Text File
    arcpy.CalculateField_management(PeterTable, "CleanStreetsPeter55", expression)

    #Search Cursor to extract Peter Text Field
    myOutputFile = open("C:\PeterScripts\CleanStreets\CleanStreets55.txt", 'w')
    rows = arcpy.da.SearchCursor(PeterTable, ["CleanStreetsPeter55"])
