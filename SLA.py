...
gm = TlsSMTPHandler(("smtp.gmail.com", 587), 'geoffreywestgis@gmail.com', ['geoffreywestgis@gmail.com'], 'Peter Error South LA found!', ('geoffreywestgis@gmail.com', 'pythonheat1'))
...
#Send Field to Text File
    ...
    PeterTable = "C:\PeterText.gdb\SLATable"
    Outable = "SLATable"
    ...
    #Selection Query
    Harborselection = "CYLA_DISTRICT = 'SC' AND PlannedDate = CONVERT(DATE, GETDATE()) AND RESOLUTION_CODE <> '0' AND RESOLUTION_CODE IS NOT NULL AND RESOLUTION_CODE <> '-1'"
    ...
    myOutputFile = open("C:\PeterScripts\SLA\Peter.txt", 'w')
    rows = arcpy.da.SearchCursor("C:\PeterText.gdb\SLATable", ["PtrText"])
    for row in rows:
        myOutputFile.write(str(row[0]) + '\n')
    del row, rows
    myOutputFile.close()

    rowssent = arcpy.GetCount_management("C:\PeterText.gdb\SLATable")
    ...
    ATTACHMENTS = ["C:\PeterScripts\SLA\Peter.txt"]
    send_from='geoffreywestgis@gmail.com'
    send_to=['geoffreywestgis@gmail.com','James.E.Moore@lacity.org ', 'sal.aguilar@lacity.org', 'maria.maldonado@lacity.org', 'wayne.watson@lacity.org', 'nafeesah.potts@lacity.org','adrian.tornero@lacity.org', 'luis.moreno@lacity.org']
    subject='SLA Peter.Txt' + ' ' + date
